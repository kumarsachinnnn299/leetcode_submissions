...
class Solution{
  public:
    ...
    void helper(Node*root,vector<int>&arr,int level,bool left)
    {
        if(root==NULL)return;
        ...
        if(left)
        {
            helper(root->left,arr,level+1,left);
            helper(root->right,arr,level+1,left);
        }
        else{
            helper(root->right,arr,level+1,left);
             helper(root->left,arr,level+1,left);
